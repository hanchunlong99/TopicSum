...
class VAE(nn.Module):
    def __init__(self, encode_dims, dropout):
        ...
        self.in_features = encode_dims[0]  # 输入句子的数量 500
        self.n_topic = encode_dims[-1]  # 主题数量

        self.fc1 = nn.Linear(200, 1)
        self.fc2 = nn.Linear(self.in_features, 200)

        self.rho = nn.Linear(200, self.in_features)  # num_sent * 200
        self.alpha = nn.Linear(200, self.n_topic)

        self.norm_input = self.norm_input = nn.LayerNorm(400)
